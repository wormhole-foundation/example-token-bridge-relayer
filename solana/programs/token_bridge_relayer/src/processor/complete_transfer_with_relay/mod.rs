...
use crate::{
    ...
    state::{RedeemerConfig, RegisteredToken},
};
...
pub struct RedeemToken<'ctx, 'info> {
    ...
    registered_token: &'ctx Account<'info, RegisteredToken>,
    native_registered_token: &'ctx Account<'info, RegisteredToken>,
...
pub fn redeem_token(
    ...
    if payer.key() == recipient.key() {
        ...
        // recipient.
        let (token_amount_in, native_amount_out) = registered_token
