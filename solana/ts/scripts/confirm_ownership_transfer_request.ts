...
async function confirmOwnershipTransferRequest(
  connection: Connection,
  payer: Keypair
) {
  // Create the submit ownership transfer request transaction.
  const confirmOwnershipTransferRequestIx =
    await tokenBridgeRelayer.createConfirmOwnershipTransferInstruction(
      connection,
  ...
  const tx = await sendAndConfirmIx(
    connection,
    confirmOwnershipTransferRequestIx,
    payer
  );
